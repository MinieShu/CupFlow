...
5. 扫描或录入一个订单并下发。眼镜端显示订单与第一步，说“开始制作”或轻触开始后进入自动识别。
6. 手机端对格架小料、独立标签物料和操作步骤进行视觉判断；异常会留存关键帧，并将提醒回传眼镜。

店长端也支持导入 `{"id":"A102","drink":"云朵乌龙奶茶","options":["少糖","去冰","加珍珠"]}` 格式的订单。导入后须由店长确认下发；识别决策日志和运行状态仅保存在手机本地。

## 注意

- 视觉模型密钥只配置在 Web 服务端，不能写入 APK 或提交到仓库。
- 店长端默认使用本地 USB 地址 `http://127.0.0.1:3000/api/vision`。跨网络部署时可在“运行状态”中配置受保护的 HTTPS 服务地址，无需改动 APK。
