...
export async function POST(request: Request) {
  ...
  const targetIngredient = /^加入(.+)$/.exec(expected)?.[1]?.trim();
  const ingredientHint = targetIngredient && !["奶", "茶", "茶底", "混合后的奶茶"].includes(targetIngredient)
    ? `当前目标小料是“${targetIngredient}”。若画面清楚显示正在加入另一种小料（例如目标是珍珠却加入椰果），必须返回 wrong，并在 reason 写出可见小料；只有实际加入目标小料时，珍珠返回 pearls，其他目标小料返回 topping。`
    : "";
  const prompt = body.mode === "label"
    ...
    : `判断奶茶制作台画面中刚发生的关键操作。当前订单为：${orderText}；当前应执行步骤是：${expected}。${earlierFrames.length ? "会先给出较早关键帧，再给出当前关键帧；必须结合两帧的变化判断动作。" : "仅有当前关键帧。"}${liquidStepHint}${vesselHint}${ingredientHint}${gridText}只根据画面中的可见动作、物料、液位或标签判断；不要因为“当前应执行步骤”而猜测已经完成，不确定时返回 unknown。`;
