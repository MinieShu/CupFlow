...
- **订单到眼镜**：店长端可录入或扫描杯贴识别订单，并由店长确认后下发到指定眼镜；眼镜端收到有效订单后进入制作流程。
...
项目提供一条单订单本地闭环：店长端录入/扫描订单并下发 → 眼镜端显示步骤与语音提示 → 视觉服务结合配方、格架配置和关键帧核验 → 异常提示、人工纠正/跳过留痕 → 完成状态回传店长端。具体运行步骤、证据清单、数据与隐私边界见下列项目文档。
...
- [Demo 运行与验证清单](docs/project/DEMO_EVIDENCE_CHECKLIST.md)
- [数据来源与合规说明](docs/project/DATA_COMPLIANCE_NOTE.md)
- [用户反馈与量化效果记录表](docs/project/USER_FEEDBACK_AND_METRICS_TEMPLATE.md)
- [MIT License](LICENSE)
